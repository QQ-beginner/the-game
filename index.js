...
var game = {                        
    ...
    /* 显示得分 */
    ShowScore: function () {
        var gradient = context.createLinearGradient(0, 0, 120, 60);//canvas渐变操作
    ...
    ShowLife: function () {
        ...
        context.fillText("LIFE:" + this.life, 400, 50);//画布上填充文本
    ...
    NewtonHit: function (type) {
        if(type == 'fruit'){             //仅被水果砸中时需
            ...
            if (hittime == 9) {
                game.life -= 1;         //生命值-1，判断是否能够复活在 ShowLife()函数内
                fruitarr = [];
                bookarr = [];
                newtonX = myCanvas.width / 2;
            }
    ...
    HitByFruit: function () {
        for (var i = 0; i < fruitarr.length; i++) {
            if(fruitarr[i][1] + fruitarr[i][2] < newtonY - newton.height / 2 + newton.height){          
                if (fruitarr[i][0] < newtonX - newton.width / 2) {         
                ...
                } else if (fruitarr[i][0] > newtonX - newton.width / 2) {  
    ...
    HitByBook: function () {
        for (var i = 0; i < bookarr.length; i++) {
            if(bookarr[i][1] + bookarr[i][2] < newtonY - newton.height / 2 + newton.height){         
                if (bookarr[i][0] < newtonX - newton.width / 2) {        
                ...
                } else if (bookarr[i][0] > newtonX - newton.width / 2) {  
